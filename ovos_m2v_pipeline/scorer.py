...
class PrototypeScorer:
    ...
    def add_intent(
        ...
        *entities* (optional ``{entity_name: [values, ...]}``) is
        ...
        if entities:
            # OVOS-INTENT-4 §7: an entity belongs to the skill that
            # registers it, and `_entity_values` reads the owning skill's
            # own values when it fills a template. A register message with
            # no `skill_id` is malformed and is dropped with a warning, so
            # the slot would never fill.
            for name, values in entities.items():
                pipeline._handle_intent4_register_entity(Message(
                    SpecMessage.ENTITY_REGISTER.value,
                    data={"skill_id": skill_id, "entity_name": name,
                          "samples": list(values), "lang": use_lang},
                ))
