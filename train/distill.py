...
def distil_one(entry: dict, out_dir: Path, trust_remote_code: bool = False,
    ...
    if entry.get("custom_code") and not trust_remote_code:
        ...
        return None
    # the libraries are imported after the cheap skips, so a manifest that
    # skips a backbone does not need them installed for that entry
    ...
    # model2vec.distill pulls torch in; a backbone skipped above never
    # needs it, so the import sits after every skip
