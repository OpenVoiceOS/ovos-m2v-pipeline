...
#: Every key a row must carry. A row missing one cannot be checked at all.
REQUIRED_KEYS = ("label", "utterance", "kind")


def read_eval(path: Path) -> tuple:
    """The rows of *path*, and one named error for each line that is not a row.

    A malformed line is the contributor's own typo, so it is reported the way
    every other problem is reported: the file, the line number and the reason.
    Reading continues past it, because one typo must not mask the real errors
    ...
    with path.open(encoding="utf-8") as handle:
        for number, line in enumerate(handle, start=1):
            ...
            try:
                ...
                errors.append(f"{path.name} line {number} is not JSON: {exc.msg}")
                continue
            ...
            if missing:
                ...
                errors.append(f"{path.name} line {number} is missing {named}")
                continue
            rows.append(row)
    return rows, errors
...
def validate(eval_path: Path, corpus: Path) -> list:
    rows, errors = read_eval(eval_path)
