...
def _isolate_model_load(case):
    """Keep a load inside the test process.

    Dev resolves a repo id to a pinned revision over the Hub before it
    loads (`_resolve_model_revision`), and it keeps one embedding model per
    resolved path for the whole process (`_SHARED_MODELS`). A test that only
    fakes `model2vec` would still reach the network for "fake-model", and
    would still read a model another test left in the cache. The identity
    resolver is the one dev's own `tests/test_lazy_model_load.py` uses.
    """
    import ovos_m2v_pipeline as _m2v
    from ovos_m2v_pipeline import Model2VecIntentPipeline

    resolver = patch.object(Model2VecIntentPipeline, "_resolve_model_revision",
                            lambda self, path: path)
    resolver.start()
    case.addCleanup(resolver.stop)

    saved = dict(_m2v._SHARED_MODELS)
    _m2v._SHARED_MODELS.clear()

    def restore():
        _m2v._SHARED_MODELS.clear()
        _m2v._SHARED_MODELS.update(saved)
