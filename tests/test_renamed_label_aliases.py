...
RENAMES = [
    ...
    ("ovos-skill-wallpapers", "ovos-skill-wallpapers.openvoiceos",
     ...
     "make_wallpaper"),
    ("ovos-skill-wordnet", "ovos-skill-wordnet.openvoiceos",
     "definition", "ovos-skill-wordnet.openvoiceos", "search_wordnet"),
]

#: Old spellings that must NOT be aliased, and why. A relation lookup has no
#: destination whose template claims it, a split label has several, and a
#: deleted intent has none at all.
UNALIASED = [
    "ovos-skill-wordnet.openvoiceos:hyponym",
    "ovos-skill-wordnet.openvoiceos:hypernym",
    "ovos-skill-wordnet.openvoiceos:holonym",
    "ovos-skill-wordnet.openvoiceos:lemma",
    "ovos-skill-volume.openvoiceos:volume.max",
    "ovos-skill-volume.openvoiceos:volume.default",
    "ovos-skill-alerts.openvoiceos:CreateOcpAlarm",
    "ovos-skill-iss-location.openvoiceos:about",
]


def _alias_keys() -> set:
    """Read LABEL_ALIASES out of the builder's source.

    The builder imports pandas at module scope, which the coverage job does
    not install; the alias table is a literal dict and needs no runtime.
    """
...
def test_labels_without_an_honest_destination_stay_unaliased():
    aliased = [label for label in UNALIASED if label in _alias_keys()]
    assert not aliased, (
        f"{aliased} were aliased; their rows belong to no single surviving "
        f"intent, so folding them teaches a routing no template attests")
