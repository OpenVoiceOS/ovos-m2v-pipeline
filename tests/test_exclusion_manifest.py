...
def test_exclusion_manifest_arithmetic_closes_for_every_language(built):
    ...
    for lang, e in excl["languages"].items():
        ...
        # The residual assertion. The three above all hold for a language
        # with rows dropped and nothing explaining them, which is how a
        # `rows_dropped: 2` entry with an empty reason list passed the
        # suite. Every dropped row must carry a cause.
        assert sum(e["rows_dropped_by_cause"].values()) == e["rows_dropped"], (
            f"{lang}: {e['rows_dropped']} row(s) dropped, "
            f"{sum(e['rows_dropped_by_cause'].values())} accounted for "
            f"({e['rows_dropped_by_cause']})"
        )
        if e["rows_dropped_unfilled_slot"]:
            assert (e["rows_dropped_by_cause"].get("unfilled_slot")
                    == e["rows_dropped_unfilled_slot"]), lang
...
def _build_with(tmp_path, extra_files):
    """Build a corpus from the standard fixture plus `extra_files`."""
    pytest.importorskip("pandas")
    pytest.importorskip("sklearn")
    import yaml
    ...
    plugin = git_repo(ws / "plugin", files)
    cfg = {
        "version": 2, "workspace": str(ws),
        "git_sources": [
            {"id": "plugin", "kind": "plugin_intents", "family": "ocp",
             "pipeline_id": "ocp", "path": "plugin", "revision": plugin,
             "files": "**/locale/*/*.intent"},
        ...
        "skill_refs": {"refs": {}}, "skill_id_aliases": {},
        "skill_blacklist": [], "intent_blacklist": [],
        "filters": {"min_chars": 2, "max_chars": 160, "min_words": 1,
                    "drop_bare_slot": True, "drop_non_alpha": True,
                    "min_rows_per_label": 2},
        "split": {"test_size": 0.5, "seed": 42, "stratify": "label"},
        # The real sources.yaml sets this, and it is what moves the dialect
        # from `lang` to `lang_variant` mid-build. Without it the fixture
        # never exercises the rename and a test of the counting keys cannot
        # tell a correct build from a broken one.
        "publish": {"collapse_to_macro_language": True},
    }
    (ws / "golden.jsonl").write_text("", encoding="utf-8")
    sources = tmp_path / "sources.yaml"
    sources.write_text(yaml.safe_dump(cfg), encoding="utf-8")
...
def test_a_drop_with_no_unfilled_slot_still_names_its_cause(tmp_path):
    """The nl-NL shape from the review: rows dropped, no slot reason.

    it-IT ships one template twice, so the dedup stage drops the repeat.
    That drop has nothing to do with an unfilled slot, and before the fix
    the language was reported with `rows_dropped` above zero, an empty
    `unfilled_slot_reasons` and `rows_dropped_unfilled_slot: 0`, which reads
    as a broken manifest.
    """
    out = _build_with(tmp_path, {
        "pkg/locale/it-IT/meteo.intent":
            "che tempo fa oggi\nche tempo fa oggi\nche tempo fa domani\n",
    })
    excl = json.loads((out / "exclusions.json").read_text())
    it = excl["languages"]["it-IT"]
    assert it["rows_dropped"] > 0
    assert it["rows_dropped_unfilled_slot"] == 0
    assert it["unfilled_slot_reasons"] == []
    assert sum(it["rows_dropped_by_cause"].values()) == it["rows_dropped"]
    assert "unfilled_slot" not in it["rows_dropped_by_cause"]


def test_counts_survive_the_dialect_narrowing(tmp_path):
    """`rows_before` and `rows_kept` must be counted on the same key.

    Halfway through the build, `lang` is narrowed to the bare language and
    the dialect moves to `lang_variant`. Counting `lang` on both sides of
    that rename compares `en-US` before with `en` after, so every variant
    reports `rows_kept: 0`. On the real corpus that put 50 of 54 languages
    in `languages_absent_from_output`, for a corpus that carries 49 of them.
    """
    out = _build_with(tmp_path, {
        "pkg/locale/it-IT/meteo.intent":
            "che tempo fa oggi\nche tempo fa domani\n",
    ...
    en = excl["languages"]["en-US"]
    assert en["rows_kept"] > 0, (
        "en-US kept no rows, which means the after-count was read on the "
        "narrowed `lang` column while the before-count used the dialect"
    )
    assert "en-US" not in excl["languages_absent_from_output"]
