...
def built(tmp_path):
    ...
    plugin = git_repo(ws / "plugin", {
        ...
        # en-US: two malformed references, a capital name and a leading
        # digit, neither of which OVOS-INTENT-1 3.7 names -> 2 dropped
        "pkg/locale/en-US/oops.intent":
            "greet <Greeting> now\n"
            "greet <1abc> now\n"
            "greet plainly\n"
            "greet politely\n",
...
def test_malformed_reference_is_dropped_and_counted(built):
    """OVOS-INTENT-1 3.7 names a reference in lowercase letters, digits and
    underscores. `<Greeting>` and `<1abc>` are not references, so their lines
    are malformed (3.6) and must not ship as literal text."""
    utterances = {r["utterance"] for r in _rows(built)}
    # positive control: the well-formed lines of the same file are kept
    assert {"greet plainly", "greet politely"} <= utterances
    assert not any("greeting" in u.lower() and u.startswith("greet ") and
                   "<" in u for u in utterances)
    assert not any(u.startswith("greet ") and u not in
                   {"greet plainly", "greet politely"} for u in utterances)
    manifest = json.loads((built / "manifest.json").read_text())
    assert manifest["dropped_malformed_voc_rows"] == 2
